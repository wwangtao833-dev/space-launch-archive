...
function apply(){
  ...
  let rows=state.all.filter(x=>{
    ...
    return (!q||searchable(x).includes(q))
...
function render(){
  ...
  rows.forEach(x=>{
    ...
    (x.sources||[]).forEach((src,i)=>{const a=document.createElement("a");a.href=src.url;a.target="_blank";a.rel="noopener noreferrer";a.textContent=src.title||("来源 "+(i+1));s.appendChild(a);});
